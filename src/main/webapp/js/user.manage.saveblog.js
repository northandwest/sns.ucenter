	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.result=="200"){
				...
				$("#keyWord").val("");
				editor.setContent("");
				setTimeout("parent.document.location.href='../user/"+data.userId+"/blog/"+data.blogId+"'",1000);
			}else{
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.result=="200"){
				...
				setTimeout("parent.parent.document.location.href='../user/"+data.userId+"/blog/"+data.blogId+"'",2000);
			}else{
				ulewo_common.alert(data.msg,2);
			}
		}
