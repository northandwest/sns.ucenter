...
import com.ulewo.dao.UlewoUserDao;
...
import com.ulewo.util.DateUtil;
...
//			userMapper.insert(user);
	...
	private User checkUserName(String userName) {
		...
		User user = null;// userMapper.selectBaseInfo(map);
		return user;
	}
	...
	public UserVo4Api api_login(Map<String, String> map)
		...
		if (StringUtils.isEmpty(account)) {
			...
			if (null == user || !user.getPassword().equals(password)) {
				...
				userVo.setPrevisitTime(user.getPreVisitTime().toLocaleString());
				userVo.setRegisterTime(user.getPreVisitTime().toLocaleString());
				userVo.setMark(user.getMark());
				return userVo;
			}
		}
